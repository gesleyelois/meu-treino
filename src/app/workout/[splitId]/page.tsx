...
export default function ActiveWorkout() {
    ...
    const finishWorkout = async () => {
        ...
        // Generate a UUID without the crypto module
    ...
    const progress =
        exerciseStates.reduce(
            (acc, es) => acc + es.sets.filter((s) => s.completed).length,
            0
        ) /
            ...
            <div className="px-4 max-w-lg mx-auto mt-4">
                ...
                <div className="mb-4">
                    ...
                    <p className="text-sm text-zinc-400 mt-0.5">
                        {currentWE.exercise.muscleGroup} · {currentWE.sets}×{currentWE.targetReps} ·{" "}
                        {currentWE.restTimeSeconds}s descanso
                    </p>
                </div>
                ...
                {showTimer && !allSetsCompleted && (
                    <div className="mb-6 bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
                        <h3 className="text-sm font-medium text-zinc-400 text-center mb-3">
                            ⏱ Tempo de Descanso
                        </h3>
                        <RestTimer
                            seconds={currentWE.restTimeSeconds}
                            autoStart={true}
                ...
                <div className="flex gap-3 mt-4">
                    {currentExerciseIndex > 0 && (
                        <button
                            ...
                            className="flex-1 h-14 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded-xl 
                ...
                {isLastExercise && allSetsCompleted && (
                    <button
                        ...
                        disabled={saving}
                        className="w-full mt-4 h-16 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl 
                       font-bold text-lg transition-all active:scale-[0.97]
                ...
                {!allExercisesCompleted && (
                    <button
                        onClick={finishWorkout}
                        disabled={saving}
                        className="w-full mt-3 h-12 bg-transparent hover:bg-zinc-900 text-zinc-500 hover:text-zinc-300 
                       rounded-xl font-medium text-sm transition-colors border border-zinc-800/50"
                    >
